    ...
    case 'L':
    {
      ...
      else
      {
        ...
        if (winners.empty())
        {
          ...
          Sleep(500);
          std::cout << " Komora maszyny losującej jest pusta, zwolnienie blokady za ";
          for (int i(5); i > 0; i--)
          {
            pos.X = 60;
            pos.Y = 5 + temp;
          ...
          do //tasowanie graczy bez powtorzen
          {
            ...
            if (a == temp)
              a++;
            if (b == temp)
              b--; //zawężanie przedziału losowania (na końcach)
            if (tempNum.at(temp))
            {
          ...
          std::cout << " Losowanie już się odbyło!\a" << std::endl;
...
void lottoLogusReplay(std::vector<std::string> &nicknames, std::vector<short> &playersId, std::vector<std::string> &winners, std::vector<short> &winnersId)
{
  ...
  if (winners.empty())
  {
    std::cout << "Brak zapisu losowania!\a";
    Sleep(500);
  }
  else if (nicknames.size() != playersId.size())
  {
    std::cout << "Dokonano zmian w liście zapisanych!\a";
    Sleep(500);
  } //fix crash program
  else
  {
    ...
    for (int i(0); i < winnersId.size(); i++)
      if (i % 2 == 1)
        playersId.at(winnersId.at(i - 1)) = winnersId.at(i);
    ...
    while (licznik <= count)
    {
      ...
      while (winners.size() < 4)
      {
        ...
        if (playersId.at(temp))
        {
          ...
          if (playersId.at(temp) == randoms.at(0))
            randomsC[0]++;
          if (playersId.at(temp) == randoms.at(1))
            randomsC[1]++;
          if (playersId.at(temp) == randoms.at(2))
            randomsC[2]++;
          if (playersId.at(temp) == randoms.at(3))
            randomsC[3]++;
          if (playersId.at(temp) % 6 == 0)
            randomsC[4]++; //pusty los
