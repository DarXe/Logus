...
{
  if (Reconnect(line) || Quit(line) || StartTimer(line) || SetNick(line) || SetTrack(line) || SetTimer(line) || AddNickname(line) || DelNickname(line) || SetMoney(line) || SetCourses(line) || SetLoadingTime(line) || Reset(line) || HardReset(line) || FindTransfers(line) || FindWord(line) || OpenConfig(line) || OpenConsoleLog(line) || OpenLogusLog(line) || Timestamp(line) || RenderEngine(line) || ClearChat(line) || SetMax(line) || SetMin(line) || SetRefresh(line) || SetDynamicRefresh(line) || AutoReconnect(line))
    return 1;
  else
    return 0;
}
